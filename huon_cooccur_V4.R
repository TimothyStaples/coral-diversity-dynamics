...
xlims=list(c(0,0.5), c(0,0.5))
ylims=list(c(0,0.5), c(0,0.5))
...
ylims=list(c(3.5,5.5),c(2,4.5),c(1.5,4), c(0,0.7), c(0,0.6))
...
sapply(1:length(alpha.model.list), function(n1){
  ...
  fit1.ylims<-rbind(c(2.75,5.75), c(1.75,4.75), c(1.5,4.75))[n1,]
...
huon_site_re <- huon_site
huon_site_re$locality <- relevel(huon_site_re$locality, ref="Midway Cove")
red <- dbrda(genusMatProp ~ (transect.age * locality), data=huon_site_re)
sppscores(red) <- genusMatProp
redAge <- dbrda(genusMatProp ~ transect.age, data=huon_site_re)

redGr <- dbrda(grMatProp ~ (transect.age * locality), data=huon_site_re)
sppscores(redGr) <- grMatProp

redGrAge <-  dbrda(grMatProp ~ transect.age, data=huon_site_re)
plot(redGr)

#                   rda plot ####

pdf(date.wrap("./plots/redundancy_GENUS", ".pdf"), height=3.5, width=7)
par(oma=c(1,0,0.5,0.5), mar=c(1,2.5,0,0), ps=10, tcl=-0.25, mgp=c(3,0.5,0), las=1, mfrow=c(1,2))

redLoad <- red$CCA$biplot[,1:2]
redPos <- scores(red)$sites
redSp <- scores(red)$species
cap.imp <- summary(red)$cont$importance[,1:2]
RsquareAdj(red)
anova.cca(red, step = 1000, by="terms")

plot(redPos, asp=1, xlab="", ylab="",type="n", xaxt="n", xlim=c(-4,4))

axis(side=1, mgp=c(3,0.1,0))
mtext(side=1, line=1, text=paste0("RDA1 (",round(cap.imp[2,1]*100,2),"%)"))
mtext(side=2, line=1.5, text=paste0("RDA2 (",round(cap.imp[2,2]*100,2),"%)"), las =0)

points(redSp, col="grey30", cex=0.8, pch=4)

spMag <- sqrt(rowSums(redSp^2))
text(redSp[spMag > 0.2,], labels=rownames(redSp)[spMag > 0.2],
     col="grey30", pos=3, font=3)

points(redPos, 
       pch=21, 
       bg=c("red", "darkgreen", "darkgreen", "darkgreen", "blue",
            "blue", "blue", "red", "red")[huon_site$locality], cex=0.65)

rect(xleft=par("usr")[1], xright=par("usr")[2],
     ybottom=par("usr")[3], ytop=par("usr")[4],
     border=NA, col=rgb(1,1,1,0.5))

# sapply(1:length(huon_site$locality), function(n){
# 
#   subAge <- huon_site[huon_site$locality == locs[locs.order[n]],]
#   subPos <- redPos[as.character(subAge$transect),]
#   subPos <- subPos[order(subAge$transect.age, decreasing=TRUE),]
#   lines(subPos)
# })

# add in x's for site centroids
ageEff <- redLoad[grepl("age", rownames(redLoad)),]
rownames(ageEff) = c("Midway Cove",
                     substr(rownames(ageEff)[-1],
                            nchar("transect.age:locality")+1,
                            nchar(rownames(ageEff)[-1])))
ageEff <- ageEff[locs[locs.order],]

siteEff <- redLoad[!grepl("age", rownames(redLoad)),]
rownames(siteEff) = substr(rownames(siteEff),
                            nchar("locality")+1,
                            nchar(rownames(siteEff)))
siteEff <- siteEff[locs[locs.order][-9],]

arrows(x0=0, y0=0, 
       x1=ageEff[,1] * 6, 
       y1=ageEff[,2] * 6, 
       col=c(rep("darkgreen", 3), rep("red", 3), rep("blue",3)),
       length=0.1,
       lwd=1.5)

text(ageEff * 6, pos=c(3,3,3,4,4,4,1,3,2), offset=0.35,
     labels=9:1, 
     col=c(rep("darkgreen", 3), rep("red", 3), rep("blue",3)))

text(x=relative.axis.point(0.01, "x"),
     y=relative.axis.point(0.96, "y"), adj=0,
     labels=("(A) Genus composition"), font=2)


redLoad <- redGr$CCA$biplot[,1:2]
redPos <- scores(redGr)$sites
redSp <- scores(redGr)$species
cap.imp <- summary(redGr)$cont$importance[,1:2]
RsquareAdj(redGr)

grNames <- c("Branching", "Stout branching", "Encrusting", "Massive", "Tabular")

plot(redPos, asp=1, xlab="", ylab="",type="n", xaxt="n", xlim=c(-3,3))
...
text(ageEff * 6, pos=c(2,2,2,3,3,1,4,4,3), offset=0.35,
     labels=9:1, 
     col=c(rep("darkgreen", 3), rep("red", 3), rep("blue",3)))

text(x=relative.axis.point(0.01, "x"),
     y=relative.axis.point(0.96, "y"), adj=0,
     labels=("(B) Growth form composition"), font=2)
